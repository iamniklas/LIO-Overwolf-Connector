...
class InGame extends AppWindow {
  ...
  private onInfoUpdates(info) {
    this.logLine(this._infoLog, "Lol", true);
    this.logLine(this._infoLog, info, false);
  }

  private fail(event) {
    this.logLine(this._eventsLog, "Failed", true);
  }
  ...
  private onNewEvents(e) {
    ...
    this.logLine(this._eventsLog, "Sending", true);

    // var webSocket = new WebSocket("localhost:2400");
    // webSocket.onerror = this.fail(null);
    // webSocket.send(JSON.stringify(e.events));
    // webSocket.close();
